...
export const GET = withApi(async () => {
  ...
  const [
    ...
    d.get<{ n: number }>("SELECT COUNT(*) AS n FROM slack_notifications WHERE read = 0 AND important = 1"),
    d.get<{ n: number }>("SELECT COUNT(*) AS n FROM mail_digest WHERE read = 0 AND important = 1"),
    d.get<{ n: number }>("SELECT COUNT(*) AS n FROM letters WHERE status IN ('neu','aktion')"),
    d.get<{ n: number }>("SELECT COUNT(*) AS n FROM watcher_events WHERE seen = 0"),
  ...
  return NextResponse.json({
    ...
    counts: {
      slack: Number(slackCountRow?.n ?? 0),
      mail: Number(mailCountRow?.n ?? 0),
      letters: Number(letterCountRow?.n ?? 0),
      watcher: Number(watcherCountRow?.n ?? 0),
    },
    tasksToday,
    habits,
    habitLogsToday,
